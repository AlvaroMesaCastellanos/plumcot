...
classmatrix=np.eye(4, dtype=float) ##numpy array of classes
firstvector=1
		...
		if 'Episode' in fich: # if series we process it		
			...
			for item in classarray:
				if not 'X' in item and notnamebefore:##its a labeled word and the previous one wasnt a noun		
					...
					if '1' in item:
						Yaux=(classmatrix[0][:])			
					elif '2' in item: #2class before
						Yaux=(classmatrix[1][:])
					elif '3' in item or not item: #if its 2class after or empty
						Yaux=(classmatrix[2][:])
					elif '4' in item or not item: #if its 3 class or empty
						Yaux=(classmatrix[3][:])
					if firstvector:
						X=vec
						Y=Yaux
...
print(X.shape)
print(Y.shape)
np.save('/vol/work/mesa/wemb4classes/X'+saveword+numwords,X)
np.save('/vol/work/mesa/wemb4classes/Y'+saveword+numwords,Y)
