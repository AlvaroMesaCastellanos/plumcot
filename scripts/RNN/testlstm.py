...
dire='/vol/work/mesa/LSTMearlystopping/'
emblist=list(range(1,5))
...
for emb in emblist:
	...
	if True:
		f=open('/vol/work/mesa/LSTMearlystopping/neuronsvscontext','a')
	...
	plt.grid()
	plt.title('Accuracy on training, LSTMearlystopping '+str(emb)+' context')
	plt.ylabel('accuracy')
	plt.xlabel('epoch')
	#plt.xlim(0.0, 100.0)
	legendlist=['train','max dev accuracy = '+str(bestacc)]
	plt.legend(legendlist, loc='best')
	plt.savefig('/people/mesa/Desktop/results/LSTMearlystopping/'+str(emb)+'.png')
	plt.clf()
	plt.cla()
	plt.close()
	...
	if True:
		...
		dim=Xtest.shape
		Xtest=Xtest.reshape(dim[0],2*emb,300)
		...
		f=open('/people/mesa/Desktop/results/LSTMearlystopping/confusionmatrix','a')
