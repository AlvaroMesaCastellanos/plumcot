...
NNmodels='/vol/work/mesa/NNearlystoppingweights4classes/'
LSTMmodels='/vol/work/mesa/LSTMearlystoppingweights4classes/'
...
for modeltype in modelslist: #we evaluate results on NN and LSTM models
	if 'LSTM' in modeltype:
		f=open('/vol/work/mesa/RESULTS/resultsLSTMearlystoppingweights4classesclusters','a')
	elif 'NN' in modeltype:
		f=open('/vol/work/mesa/RESULTS/resultsNNearlystoppingweights4classesclusters','a')
			...
			for nneurons in listdir(modeltype+nembedings):
				...
				for testfile in listdir(testdire):
									...
									if namebefore:
										...
										if 'None' in norm_name: #we didnt find it
											...
											if predicted_class==2: #2nd person after
												if firstname2after:
													classed_name2aft[nlines]=clusterdict.get(norm_name,'None') ##we assign the cluster
											...
											if predicted_class==1: #2nd person before
												if firstname2before:
													classed_name2bef[nlines]=clusterdict.get(norm_name,'None') ##we assign the cluster
					...
					for element in lines_names:
						...
						elif element in classed_name2bef:
							classed_name2bef[index]=element
						elif element in classed_name2aft:
							classed_name2aft[index]=element
