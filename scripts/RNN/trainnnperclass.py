...
for embeding in embedingslist:
	for classes in classlist:
		...
		# Compile model
		model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy']) #we chose binary_crossentropy as its a class classification
		# Load X,Y and Fit the model
		X=np.load('/vol/work/mesa/wembperclass/X'+classes+'train'+str(embeding)+'.npy')
		Y=np.load('/vol/work/mesa/wembperclass/Y'+classes+'train'+str(embeding)+'.npy')
		Xdev=np.load('/vol/work/mesa/wembperclass/X'+classes+'dev'+str(embeding)+'.npy')
		Ydev=np.load('/vol/work/mesa/wembperclass/Y'+classes+'dev'+str(embeding)+'.npy')
		# Train the model with train and create folder to store the models
		path='/vol/work/mesa/NNearlystoppingperclass/'+str(embeding)+'.embeding/'+classes+'flayerneurons.'+str(neuronsn)
