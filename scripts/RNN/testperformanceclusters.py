...
NNmodels='/vol/work/mesa/NNearlystopping/'
LSTMmodels='/vol/work/mesa/LSTMearlystopping/'
...
for modeltype in modelslist: #we evaluate results on NN and LSTM models
	...
	#	f=open('/vol/work/mesa/RESULTS/resultsLSTMearlystoppingnoweightsclusters','a')
	#elif 'NN' in modeltype:
	#	f=open('/vol/work/mesa/RESULTS/resultsNNearlystoppingnoweightsclusters','a')
		...
		if 'embeding' in nembedings:	
			for nneurons in listdir(modeltype+nembedings):
				...
				for testfile in listdir(testdire):
					...
					scores1=[]
					scores2=[]
					lines_names=[] ##list where we will store the clusters
					nlines=-1
					for line in ftest:
						if 'Scene' in line: ##we pass scene information
							...
							characterdict=DICT.get(character,'None')
							lines_names.append(clusterdict.get(characterdict,'None'))
				...
				print('1 '+str(contextaux[0])+' '+str(layeraux)+' '+str(pacc1)+' '+str(precision1)+' '+str(rapel1)+' '+str(fscore1a)+' '+str(fscore1b)+' '+str(fscore1c)+' '+str(pacc)+' '+str(totallines)+' '+str(int(pacc)/int(totallines))+' '+str(int(pacc)/int(totalprec))+'\n')
				print('2 '+str(contextaux[0])+' '+str(layeraux)+' '+str(pacc2)+' '+str(precision2)+' '+str(rapel2)+' '+str(fscore2a)+' '+str(fscore2b)+' '+str(fscore2c)+' '+str(pacc)+' '+str(totallines)+' '+str(int(pacc)/int(totallines))+' '+str(int(pacc)/int(totalprec))+'\n')	
				print('12 '+str(contextaux[0])+' '+str(layeraux)+' '+str(pacc)+' '+str(precision)+' '+str(rapel)+' '+str(fscorea)+' '+str(fscoreb)+' '+str(fscorec)+' '+str(pacc)+' '+str(totallines)+' '+str(int(pacc)/int(totallines))+' '+str(int(pacc)/int(totalprec))+'\n')	
